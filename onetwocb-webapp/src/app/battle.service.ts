...
export class BattleService {
  ...
  private readonly snapshot: BehaviorSubject<GameSnapshot> = new BehaviorSubject(
    this.defaultGameSnapshot(),
  );
  ...
  undo() {
    ...
    if (this.history[this.history.length - 1].winner === 'player1') {
      if (curr.player2) {
        ...
        const p1 = snapshot.player1.characters.find(
          (r) => r.name === curr.player1.name,
        );
        const p2 = snapshot.player2.characters.find(
          (r) => r.name === h.player2Character,
        );
        this.currentRound.next({
          player1: p1,
          player2: p2,
    ...
    if (this.history[this.history.length - 1].winner === 'player2') {
      if (curr.player1) {
        ...
        this.snapshot.next(snapshot);
        const p1 = snapshot.player1.characters.find(
          (r) => r.name === h.player1Character,
        );
        const p2 = snapshot.player2.characters.find(
          (r) => r.name === curr.player2.name,
        );
        this.currentRound.next({
          player1: p1,
          player2: p2,
  ...
  private buildSnapshot(): GameSnapshot {
    ...
    for (const round of this.history) {
      if (round.winner === 'player1') {
        snapshot.player1.characters.find(
          (char) => char.name === round.player1Character,
        ).stocks = round.remainingStocks;
        snapshot.player2.characters.find(
          (char) => char.name === round.player2Character,
        ).stocks = 0;
  ...
  submitRound(remainingStocks: number) {
    ...
    if (inProgress.winner === 'player1') {
      nextRound.player1 = {
        name: inProgress.player1.name,
        stocks: remainingStocks,
      };
    } else {
      nextRound.player2 = {
        name: inProgress.player2.name,
        stocks: remainingStocks,
      };
    }
  ...
  loadHistory(history: Round[]) {
    ...
    if (lastRound.winner === 'player1') {
      nextRound.player1 = {
        name: lastRound.player1Character,
        stocks: lastRound.remainingStocks,
      };
    } else {
      nextRound.player2 = {
        name: lastRound.player2Character,
        stocks: lastRound.remainingStocks,
      };
    }
